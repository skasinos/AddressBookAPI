...
class TestModel(APITestCase):
    def test_creates_user(self):
        user = User.objects.create_user("userx", "userx@gmail.com", "password123!@")
        self.assertIsInstance(user, User)
        self.assertFalse(user.is_staff)
        self.assertEqual(user.email, "userx@gmail.com")
    ...
    def test_raises_error_with_message_when_no_username_supplied(self):
        with self.assertRaisesMessage(ValueError, "The given username must be set"):
            User.objects.create_user(
                username="", email="userx@gmail.com", password="password123!@"
            )
    ...
    def test_raises_error_with_message_when_no_email_supplied(self):
        with self.assertRaisesMessage(ValueError, "The given email must be set"):
            User.objects.create_user(
                username="userx", email="", password="password123!@"
            )
    ...
    def test_creates_super_user(self):
        user = User.objects.create_superuser(
            "userx", "userx@gmail.com", "password123!@"
        )
        self.assertIsInstance(user, User)
